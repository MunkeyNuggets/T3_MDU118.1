...
{
	// if the font with the unique name is already present then just return true indicate that it's present
	FontMap::iterator it = fontMap.find(uniqueName);
	if (it != fontMap.end())
		return true;
...
{
	// if the image with the unique name is already present then just return it
	ImageMap::iterator it = imageMap.find(uniqueName);
	if (it != imageMap.end())
		return it->second;
